...
final module = BuiltInModule("color", functions: [
  ...
  BuiltInCallable.overloadedFunction("hwb", {
    r"$hue, $whiteness, $blackness, $alpha: 1": (arguments) =>
        _colorFromChannels(
            ColorSpace.rgb,
            arguments[0].assertNumber("hue"),
            arguments[1].assertNumber("whiteness"),
            arguments[2].assertNumber("blackness"),
            _percentageOrUnitless(
                    arguments[3].assertNumber("alpha"), 1, "alpha")
                .clamp(0, 1)),
...
Value _invert(List<Value> arguments) {
  ...
  var space = ColorSpace.fromName(
      (arguments[3].assertString('space')..assertUnquoted('space')).text,
      'space');
...
double _adjustChannel(ColorSpace space, ColorChannel channel, double oldValue,
  ...
  if (channel is LinearChannel) {
    var result = oldValue +
        _percentageOrUnitless(adjustmentArg, channel.max, channel.name);
    return space.isStrictlyBounded
        ? fuzzyClamp(result, channel.min, channel.max)
        : result;
  } else {
    return oldValue + adjustmentArg.convertValueToUnit('deg', channel.name);
  }
}
...
Value _rgbTwoArg(String name, List<Value> arguments) {
  ...
  if (second.isSpecialNumber) {
    return SassString(
        "$name(${color.channel('red')}, ${color.channel('green')}, "
        "${color.channel('blue')}, ${arguments[1].toCssString()})",
        quotes: false);
  }
...
Value _parseChannels(String functionName, Value input,
    ...
    _maybeParseChannels(input, space: space, name: name) ??
    _functionString(functionName, [input]);

/// Like [_parseColorComponents], but returns `null` if [input] contains a
/// construct that can only be resolved at browse time.
SassColor? _maybeParseChannels(Value input, {ColorSpace? space, String? name}) {
  if (input.isVar) return null;
  ...
  if (componentList.isEmpty) {
    ...
    // We just set this to avoid static analysis checks. If components is a
    // variable, we return below as soon as we verify that `alphaValue` is
    // valid.
    channels = [];
  } else {
    if (space == null) {
    ...
    for (var channel in channels) {
      // TODO: handle none
      if (!channel.isSpecialNumber && channel is! SassNumber) {
        throw SassScriptException(
            'Expected channel $channel to be a number.', name);
      }
    }
  }

  if (alphaValue?.isSpecialNumber ?? false) return null;
  var alpha = alphaValue == null
      ? 1.0
      : _percentageOrUnitless(alphaValue.assertNumber(name), 1, name)
          .clamp(0, 1)
          .toDouble();
  ...
  return _colorFromChannels(space, channels[0] as SassNumber,
      channels[1] as SassNumber, channels[2] as SassNumber, alpha);
}
...
SassColor _colorFromChannels(ColorSpace space, SassNumber channel0,
  ...
  switch (space) {
    case ColorSpace.hsl:
      _checkAngle(channel0, 'hue');
      _checkPercent(channel1, 'saturation');
      _checkPercent(channel2, 'lightness');
...
double _channelFromValue(ColorChannel channel, SassNumber value) =>
    channel is LinearChannel
        ...
        : value.coerceValueToUnit('deg', channel.name);
