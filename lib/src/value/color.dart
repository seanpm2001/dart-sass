...
class SassColor extends Value {
  ...
  ///
  /// The semantics of this depend on the color space.
  ///
  /// @nodoc
  @internal
  final double channel0;
  ...
  factory SassColor.forSpace(ColorSpace space, List<double> channels,
    ...
    if (channels.length != space.channels.length) {
      ...
      return SassColor.forSpaceInternal(
          space,
          channels[0],
          clampChannels ? fuzzyClamp(channels[1], 0, 100) : channels[1],
          clampChannels ? fuzzyClamp(channels[2], 0, 100) : channels[2],
          alpha);
    }
  }
  ...
  @internal
  SassColor.forSpaceInternal(
      this._space, this.channel0, this.channel1, this.channel2, double? alpha,
      [this.format])
    ...
    assert(
        ...
        "[BUG] Tried to create "
        "$_space($channel0, $channel1, $channel2)");
  ...
  SassColor interpolate(SassColor other, InterpolationMethod method,
    ...
    var mixed0 =
        (color1.channel0 * thisMultiplier + color2.channel0 * otherMultiplier) /
            mixedAlpha;
    var mixed1 =
        (color1.channel1 * thisMultiplier + color2.channel1 * otherMultiplier) /
            mixedAlpha;
    var mixed2 =
        (color1.channel2 * thisMultiplier + color2.channel2 * otherMultiplier) /
            mixedAlpha;
    ...
    switch (method.space) {
      ...
      case ColorSpace.hwb:
        mixed = SassColor.forSpaceInternal(
            method.space,
            _interpolateHues(
                color1.channel0, color2.channel0, method.hue!, weight),
            mixed1,
            mixed2,
            mixedAlpha);
        break;
      ...
      case ColorSpace.oklch:
        mixed = SassColor.forSpaceInternal(
            ...
            mixed1,
            _interpolateHues(
                color1.channel2, color2.channel2, method.hue!, weight),
            mixedAlpha);
        break;
